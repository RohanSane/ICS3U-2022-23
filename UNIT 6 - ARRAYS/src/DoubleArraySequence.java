...
public class DoubleArraySequence implements Cloneable {
   ...
   public void addAfter(double d) {
      ...
      if(currentIndex == 0 && manyItems == 0){
         ...
         src[currentIndex + 1] = d;
         }

         for (int i = currentIndex + 2; i < src.length; i++) {
            src[i] = data[i - 1];
         }
   ...
   public void addBefore(double element) {
      ...
      if(currentIndex == manyItems){
         ...
         if(currentIndex == 0){
            ...
            double q = data[currentIndex];
            src[currentIndex] = element;

            for (int i = currentIndex + 1; i < src.length; i++) {
               if(i == currentIndex + 1){
                  src[i] = q;
               }
               src[i] = data[i - 1];
   ...
   public void addAll(DoubleArraySequence addend) {
      if (addend == null)
         throw new NullPointerException("DoubleArraySequence cannot be equal to null");

      ensureCapacity(manyItems + addend.manyItems);

      double [] new1 = new double[data.length];

      double [] src = new double[addend.manyItems];
   ...
   public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
      ...
      DoubleArraySequence cat = new DoubleArraySequence();
      cat.data = new double[s1.manyItems + s2.manyItems];
      cat.manyItems = s1.manyItems + s2.manyItems;
      cat.currentIndex = cat.manyItems;

      for (int i = 0; i < s1.manyItems; i++) {
         cat.data[i] = s1.data[i];
      }

      for (int i = 0; i < s2.manyItems; i++) {
         cat.data[i + s1.manyItems] = s2.data[i];
      }
   ...
   public void ensureCapacity(int minimumCapacity) {
      ...
      if (minimumCapacity > data.length) {
         double [] sub = new double[minimumCapacity];
   ...
   public void removeCurrent() {
      ...
      double [] sub = new double[data.length];
